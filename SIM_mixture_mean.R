...
set.seed(123)
nobs <- 50
z1 <- mvrnorm(n = 0.5*nobs, mu = rep(2, n), Sigma = 0.5^2*Sigma1) + rnorm(nobs*0.5*nrow(grid), sd = 0.1)
z2 <- mvrnorm(n = 0.5*nobs, mu = rep(-2, n), Sigma = 0.5^2*Sigma1) + rnorm(nobs*0.5*nrow(grid), sd = 0.1)
z <- rbind(z1,z2)
# Assume z is 25 × 900, coords is 900 × 2
vis_idx <- sample(1:50,25)
df <- as.data.frame(z[vis_idx, ])
colnames(df) <- paste0("V", 1:ncol(df))
df$Response <- paste0("Row_", 1:25)

# Convert to long format
long_df <- pivot_longer(df, cols = -Response, names_to = "point_id", values_to = "Value")
long_df$point_id <- gsub("V", "", long_df$point_id)
long_df$point_id <- as.character(as.numeric(long_df$point_id))
...
# Join value and spatial location
...
save(list = ls(), file = "mixture_mean.RData")
