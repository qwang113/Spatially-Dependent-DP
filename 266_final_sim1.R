...
Sigma1 = exp(-5*paired_dist_all) 
Sigma2 = exp(-5*paired_dist_all)
...
set.seed(123)
nobs <- 50
z <- mvrnorm(n = nobs*0.4, mu = rep(0, n), Sigma = sigs1^2)
z <- rbind(z, mvrnorm(n = nobs*0.6, mu = rep(0, n), Sigma = sigs2^2))

# 
# ggplot(grid, aes(x = long, y = lat, fill = z)) +
#   geom_raster() +
...
real_density_vals <- density(train_dat[3, ])$y
real_density_xvals <- density(train_dat[3, ])$x
...
for (i in 1:nrow(all_theta)) {
  density_vals <- density(all_theta[i, 3, ])$y
  x_vals <- density(all_theta[i, 3, ])$x
