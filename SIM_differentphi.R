...
sim_size <- 50
long_grid <- lat_grid <- seq(from = -2, to = 2, length.out
                             1= sim_size)
...
Sigma1 = exp(-0.5*paired_dist_all) 
Sigma2 = exp(-3*paired_dist_all)
...
z <- mvrnorm(n = sum(idx==0), mu = rep(0, n), Sigma = 1*Sigma1) + rnorm(sum(idx==0)*nrow(grid), sd = 0.01)
z <- rbind(z, mvrnorm(n = sum(idx==1), mu = rep(0, n), Sigma = 1*Sigma2) + rnorm(sum(idx==1)*nrow(grid), sd = 0.01) )

ggplot() +
  geom_tile(aes(x = coords[,1], y = coords[,2], fill = z[49,])) +
...
save(list = ls(), file = "diff_phi.RData")